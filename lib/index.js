...
module.exports = StylusCompiler = (function() {
  ...
  StylusCompiler.prototype.compile = function(data, path, callback) {
    ...
    return this.getCompiler(data, function(compiler) {
      ...
      compiler = compiler.set('filename', path).set('compress', false).set('firebug', !!_this.cfg.firebug).set('linenos', !!_this.cfg.linenos).set('include css', !!_this.cfg.includeCss).include(sysPath.join(_this.config.paths.root)).include(sysPath.dirname(path)).use(nib());
      if (_this.cfg !== {}) {
